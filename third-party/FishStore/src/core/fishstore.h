...
inline uint32_t FishStore<D, A>::BatchInsert(const char* data, size_t length,
  ...
  while(A::HasNext(parser_ctx().parser)) {
    ...
    // THE FIELD IDS EXTRACTED HERE SHOULD MATCH WITH THE FIELD ID GIVEN IN
    // MAKE_INLINE_PSF. OTHERWISE FAILS __SILENTLY__.
    ...
    for(auto inline_psf_id: parser_state.ptr_inline_psf) {
      ...
      // THE FIELD IDS EXTRACTED HERE SHOULD MATCH WITH THE FIELD ID GIVEN BY
      // PARSER_FIELD.FIELD_ID(). OTHERWISE FAILS __SILENTLY__. THIS
      // INLINE_PSF_MAP IS POPULATED BY MAKE_INLINE_PSF
...
inline OperationStatus FishStore<D, A>::InternalScan(C& pending_context,
  ...
  //printf("Starting address: %ld, head address: %ld\n", address.control(), head_address.control());
  while(address >= start_addr && address >= head_address) {
    ...
    //if (kpt->prev_address == 0) {
    //        printf("Prev address is zero\n");
    //}
    Address tmp_address =
      TraceBackForMatch(pending_context, kpt->prev_address, head_address);
    //if (tmp_address.control() == 0) {
    //        printf("next address now zero, last address is %ld prev_address %ld\n", address.control(), kpt->prev_address);
    //}
    address = tmp_address;
  }

  Address x = hlog.begin_address.load();
  if(address < start_addr || address < x) {
    // Hitting the end of hash chain, scan is complete.
    //printf("HERE %ld %ld %ld\n", address.control(), start_addr, x.control());
    pending_context.Finalize();
    return OperationStatus::SUCCESS;
  } else {
#ifdef _NULL_DISK
...
inline Address FishStore<D, A>::TraceBackForMatch(C& pending_context,
  ...
  // payload.
	Address org_from_address = from_address;
	uint64_t loops = 0;
  ...
  //if (from_address.control() == 0) {
  //        printf("TBFM from address now zero, Loops: %ld, original: %ld\n", loops, org_from_address.control());
  //}
  return from_address;
}
...
uint32_t FishStore<D, A>::MakeInlinePSF(const std::vector<std::string>& fields,
  ...
  for(auto field : fields) {
    ...
    if(it != field_lookup_map.end()) {
      ...
      // THIS LINE GENRATES A COUNTER THAT DECIDES WHAT FIELD SIZE WILL BE
      // BE. IF YOU DONT REGISTER FIELDS CORRECTLY, THE FIELDS WONT LINE UP WITH
      // INLINE PSF LOOKUP
      field_id = static_cast<uint16_t>(field_names.size());
      field_names.push_back(field);
      field_lookup_map[field] = field_id;
    ...
    psf.fields.push_back(field_id);
    // printf("Field %s is given field id %d\n", field.c_str(), field_id);
  }
